...
%Ms. Krasteva
%June 7, 2013
%This application allows two players to play connect4.

%Global Variable Declaration
var x : array 0 .. 100 of int %Increasing playing field array to prevent crashing & going above bounds.
...
var subtitle : int := Font.New ("Algerian:20") %SAME ONE AS FONT1!!!
...
forward proc game
for i : 0 .. 100    %Provide a cycling to prevent null variables.
	x (i) := 0
end for
proc pauseProgram
	var reply : string (1)
...
proc winner
	...
	Font.Draw ("Winner!", 190, 320, title, 48)
...
proc mainMenu
	drawfillbox (0, 0, 696, 400, black)
	Font.Draw ("Play", 295, 220, font1, 48)
	Font.Draw ("Instructions", 251, 150, font1, 48)
	Font.Draw ("Exit", 305, 79, font1, 48)
	loop
	Font.Draw ("Connect4", 155, 290, title, 52)
	Mouse.Where (xValue, yValue, button) %code to find data on mouse
	if xValue >= 215 and xValue <= 455 then
		...
		if button = 1 then
			%Instructions
			drawfillbox (0, 0, 696, 400, black)
			Font.Draw ("Connect4 - Instructions", 70, 350, instructionsTitle, 48)
			Font.Draw ("Game Controls: Click on the columns, and a ball will drop.", 0, 320, font2, 52)
			Font.Draw ("Goal: Arrange 4 balls in a row (horizontally, vertically or diagonally)", 0, 280, font2, 52)
			Font.Draw ("by the same player to win.", 0, 240, font2, 52)
			Font.Draw ("Tie: If all the cells are occupied, then the game will result in a tie.", 0, 200, font2, 52)
			Font.Draw ("Press any Key to continue.", 245, 75, font2, 14)
			pauseProgram
			mainMenu
		end if
...
proc intro
	...
	for i : 1 .. 400
	drawfillbox (0, 0, 696, 400, black)
	Font.Draw ("Connect4", 560 - i, 250, title, 48)
	Font.Draw ("Programmed By: Horatiu Lazu", -255 + i, 220, subtitle, 52)
	delay (4)
	end for
	for i : 1 .. 390
